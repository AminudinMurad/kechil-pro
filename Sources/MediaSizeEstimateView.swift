        ...
        VStack(alignment: .leading, spacing: compact ? 4 : 7) {
            ...
            if let estimate {
                HStack(alignment: .firstTextBaseline, spacing: 8) {
                    VStack(alignment: .leading, spacing: 2) {
                        Text("Estimated output")
                            .font(.system(size: compact ? 8.8 : 9.5))
                    ...
                    VStack(alignment: .trailing, spacing: 2) {
                        ...
                        Text(MediaSizeText.signedBytes(estimate.deltaBytes))
                            .font(.system(size: compact ? 10.5 : 11, weight: .medium, design: .rounded))
                ...
                Text(estimate.basis.title)
                    .font(.system(size: compact ? 8.8 : 9.5, weight: .medium))
                    .foregroundStyle(Color.accentColor)
                Text(estimate.detail)
                    .font(.system(size: compact ? 8.8 : 9.3))
                    .foregroundStyle(.secondary)
